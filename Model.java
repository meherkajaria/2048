 ...
 *  @author Meher Kajaria
 *  the code is my own work from a previous semester (summer 2022)
 */
public class Model extends Observable {
    ...
    public void tilt(Side side) {
        ...
        for (int c=0; c<this._board.size(); c++) {
            boolean a[];
            a = new boolean[this._board.size()];
            for (int r = (_board.size() - 1); r >= 0; r--) {
                ...
                if (_board.tile(c, r) == null) {
                    ...
                    moveTo = r;
                    while (moveTo < _board.size()) {
                        if ((moveTo + 1) >= _board.size()) {
                            break;
                        } else {
                            if (_board.tile(c, (moveTo + 1)) == null) {
                                moveTo += 1;
                            } else {
                                break;
                            }
                    ...
                    if (moveTo == (_board.size() - 1)) {
                        _board.move(c, moveTo, _board.tile(c, r));
                        changed = true;
                    } else {
                        if (_board.tile(c, r).value() == _board.tile(c, (moveTo + 1)).value() && a[moveTo + 1] == false) {
                            _board.move(c, (moveTo + 1), _board.tile(c, r));
                            a[moveTo + 1] = true;
                            _score += _board.tile(c, (moveTo+1)).value() ;
                            changed = true;
                        }
                        else {
                            _board.move(c, moveTo, _board.tile(c, r));
